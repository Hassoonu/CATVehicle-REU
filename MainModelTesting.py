...
def main():
    ...
    while running(False, step, max_step=MAX_STEP):
        ...
        if (step > ATTACK_STEP):
            ...
            attack.notBraking(plexe, v2_data, CLAIMING_VEHICLE)
            vehicles[0].sendMessage(v2_data, vehicles[1], vehicles[0], claim_lane, trust_score.trust, step)
            trust_score.trust = vehicles[1].getTrustScore()
            append_data(v2_data)
    ...
    print(f"max accel: {vehicles[1].getMaxAcc():.2f}")
    # print(f"max distance: {vehicles[1].getMaxDistanceBetween()}")
    print(f"min distance: {vehicles[1].getMinDistanceBetween():.2f}")
    print(f"time accel stabilize: {vehicles[1].getTimeTillStable():.2f}")
